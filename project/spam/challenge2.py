gamelines = '''Game 1: 12 red, 2 green, 5 blue; 9 red, 6 green, 4 blue; 10 red, 2 green, 5 blue; 8 blue, 9 red
...
games = gamelines.split("\n")
power_sum = 0

for game in games:
    rounds = game.split(":")
    actual_rounds = rounds[1].split(";")
    cube_values = {
        "green" : 0,
        "red" : 0, 
        "blue" : 0,
    ...
    for round in actual_rounds:
        cubes = round.split(',')
        #print(cubes)

        for cube in cubes:
            cubearray = cube.split(" ")
            if (int(cubearray[1]) > cube_values[cubearray[2]]):
                cube_values[cubearray[2]] = int(cubearray[1])


    power = cube_values["blue"]*cube_values["green"]*cube_values["red"]
    power_sum += power
